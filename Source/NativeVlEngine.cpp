...
GeneratedHeapMode readGeneratedHeapMode()
{
    ...
    const auto length = GetEnvironmentVariableW(
        L"SYXG2026_VL_GENERATED_HEAP", value, static_cast<DWORD>(std::size(value)));
    if (length == 0)
        return GeneratedHeapMode::restore;
    if (length >= std::size(value))
        throw std::runtime_error("SYXG2026_VL_GENERATED_HEAP is too long");
    if (_wcsicmp(value, L"restore") == 0)
        return GeneratedHeapMode::restore;
    if (_wcsicmp(value, L"advance") == 0)
        return GeneratedHeapMode::advance;
    throw std::runtime_error(
        "SYXG2026_VL_GENERATED_HEAP must be restore or advance");
}
...
GeneratedJitterMode readGeneratedJitterMode()
{
    wchar_t value[16] {};
    const auto length = GetEnvironmentVariableW(
        L"SYXG2026_VL_GENERATED_JITTER", value,
        static_cast<DWORD>(std::size(value)));
    if (length == 0)
        return GeneratedJitterMode::zero;
    if (length >= std::size(value))
        throw std::runtime_error("SYXG2026_VL_GENERATED_JITTER is too long");
    if (_wcsicmp(value, L"zero") == 0)
        return GeneratedJitterMode::zero;
    if (_wcsicmp(value, L"native") == 0)
        return GeneratedJitterMode::native;
    throw std::runtime_error(
        "SYXG2026_VL_GENERATED_JITTER must be zero or native");
}
...
BackendCompletionMode readBackendCompletionMode()
{
    wchar_t value[16] {};
    const auto length = GetEnvironmentVariableW(
        L"SYXG2026_VL_BACKEND_COMPLETION", value,
        static_cast<DWORD>(std::size(value)));
    if (length == 0)
        return BackendCompletionMode::skip;
    if (length >= std::size(value))
        throw std::runtime_error("SYXG2026_VL_BACKEND_COMPLETION is too long");
    if (_wcsicmp(value, L"skip") == 0)
        return BackendCompletionMode::skip;
    if (_wcsicmp(value, L"zero") == 0)
        return BackendCompletionMode::zero;
    throw std::runtime_error(
        "SYXG2026_VL_BACKEND_COMPLETION must be skip or zero");
}
...
RenderPath readRenderPath()
{
    wchar_t value[16] {};
    const auto length = GetEnvironmentVariableW(
        L"SYXG2026_VL_RENDER_PATH", value, static_cast<DWORD>(std::size(value)));
    if (length == 0)
        return RenderPath::direct;
    if (length >= std::size(value))
        throw std::runtime_error("SYXG2026_VL_RENDER_PATH is too long");
    if (_wcsicmp(value, L"transport") == 0)
        return RenderPath::transport;
    if (_wcsicmp(value, L"direct") == 0)
        return RenderPath::direct;
    throw std::runtime_error(
        "SYXG2026_VL_RENDER_PATH must be transport or direct");
}
...
std::uint32_t readNativeRenderQuantum()
{
    wchar_t value[16] {};
    const auto length = GetEnvironmentVariableW(
        L"SYXG2026_VL_QUANTUM", value, static_cast<DWORD>(std::size(value)));
    if (length == 0)
        return defaultNativeRenderQuantum;
    if (length >= std::size(value))
        throw std::runtime_error("SYXG2026_VL_QUANTUM is too long");
    const auto quantum = static_cast<std::uint32_t>(_wtoi(value));
    if (quantum == 64 || quantum == 128 || quantum == 256 || quantum == 512)
        return quantum;
    throw std::runtime_error("SYXG2026_VL_QUANTUM must be 64, 128, 256, or 512");
}
                       ...
                       std::uint32_t backendTransport)
{
    ...
    const auto pathLength = GetEnvironmentVariableW(
        L"SYXG2026_VL_CODE_DUMP", pathPrefix, MAX_PATH);
                           ...
                           std::uint32_t controlRingAux)
{
    wchar_t logPath[MAX_PATH] {};
    const auto pathLength = GetEnvironmentVariableW(
        L"SYXG2026_VL_TRANSACTION_LOG", logPath, MAX_PATH);
...
public:
    ...
    void configureGeneratedAllocatorJitter()
    {
        ...
        const auto dumpPathLength = GetEnvironmentVariableW(
            L"SYXG2026_VL_CODE_DUMP", dumpPath, MAX_PATH);
